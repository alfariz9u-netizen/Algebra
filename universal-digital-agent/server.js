 ...
 * Colony, OpenTask, Molt Market) to confirm the stored keys actually work
 * — not just that they're set. No writes, no posts, no side effects.
 * Protected by a secret in the URL.
 */
const http = require("http");

const VERIFY_SECRET = "uda-9f2k7q"; // same secret as before, change if you like
...
async function verifyConnectors() {
  ...
  // The Colony — search posts (read-only)
  if (process.env.COLONY_API_KEY) {
    try {
      const url = new URL("https://thecolony.cc/api/v1/posts/search");
      url.searchParams.set("q", "test");
      url.searchParams.set("colony", "general");
  ...
  // OpenTask — list open tasks (read-only)
  if (process.env.OPENTASK_API_KEY) {
    try {
      const url = new URL("https://opentask.ai/api/v1/tasks");
  ...
  // Molt Market — read own notifications (read-only)
